...
import { STYLES, encodeAvatar, randomAvatarParts } from '../lib/avatar'
...
// campo a menos é gente que não desiste no meio. As regras moram em
// lib/contact, compartilhadas com a edição de dados da sala de espera.
...
const maxPerTable = computed(() => boot.value?.max_participants ?? 10)

// A mesa pode encher entre o toque e o "Entrar" — o poll de 2,5s é justamente
// o que faz a contagem chegar antes do erro do servidor. Se a escolhida
// fechar, ela é desmarcada: melhor a pessoa reescolher com a lista na mão do
// que descobrir no clique final.
watch(tables, (list) => {
    const chosen = list.find((t) => t.id === tableId.value)

    if (chosen?.full) tableId.value = null
})
...
const contactOk = computed(
    () => contactLooksValid(contactType.value, email.value, phoneDigits.value),
)
...
async function join() {
    ...
    try {
        const payload = await api.post('/join', {
            ...
            ...contactPayload(contactType.value, email.value, phoneDigits.value),
...
<template>
    ...
    <div v-else class="min-h-dvh flex flex-col items-center p-5 gap-5">
        ...
        <div class="w-full max-w-md rounded-3xl bg-slate-900/70 ring-1 ring-white/10 p-6 shadow-2xl backdrop-blur space-y-5">
            ...
            <ContactField
                v-model:type="contactType"
                v-model:email="email"
                v-model:phone="phoneDigits"
            />
            ...
            <AvatarBuilder v-model:parts="parts" v-model:gender="gender" />

            <div class="space-y-1.5">
                <label class="text-xs font-bold uppercase tracking-widest text-slate-400">
                    Sua mesa
                    <span class="normal-case tracking-normal font-medium text-slate-500">· até {{ maxPerTable }} pessoas</span>
                </label>
                <div class="grid grid-cols-2 gap-2 max-h-56 overflow-y-auto pr-1">
                    <button
                        ...
                        :class="tableId === table.id ? 'bg-white/10' : 'ring-white/10 bg-slate-800/60 enabled:hover:ring-white/25'"
                    ...
                    >
                        ...
                        <span class="min-w-0">
                            ...
                            <span class="block text-[11px]" :class="table.full ? 'text-amber-300 font-bold' : 'text-slate-400'">
                                {{ table.full ? 'completa' : `${table.participants_count}/${maxPerTable}` }}
                            </span>
                        </span>
                    </button>
